...
describe.skipIf(!OPENAI_API_KEY || !ANTHROPIC_API_KEY)("RAG quality eval", () => {
  it("sweeps chunk size × top-k, runs answer+judge on baseline + winner, writes results", async () => {
    // 1. Load fixtures.
    ...
    const baselineRow = retrievalRows.find(
      (r) => r.chunk === BASELINE.chunk && r.k === BASELINE.k,
    )!;
    const answerConfigs = [
    ...
    lines.push("");
    lines.push(
      `Corpus: ${docs.length} fixtures · ${questions.length} questions (${questions.filter((q) => q.negative).length} negatives).`,
    );
    ...
    for (const r of retrievalRows) {
      lines.push(
        `| ${r.chunk} | ${r.k} | ${pct(r.recall)} | ${pct(r.precision)} | ${num(r.mrr)} |`,
      );
    }
    lines.push("");
    lines.push(
      `**Retrieval winner:** chunk ${winner.chunk}, k=${winner.k} (recall ${pct(winner.recall)}, MRR ${num(winner.mrr)}). Baseline ${BASELINE.chunk}/k${BASELINE.k}: recall ${pct(baselineRow.recall)}, MRR ${num(baselineRow.mrr)}.`,
    );
    lines.push("");
