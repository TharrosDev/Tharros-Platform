...
function WeeklyGrid({
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div className="bg-card divide-border divide-y rounded-lg border shadow-xs">
        {DAYS.map((d) => {
...
function TemporaryOverrides({
  ...
  return (
    <section className="space-y-4">
      ...
      {rows.length > 0 ? (
        <ul className="bg-card divide-border divide-y rounded-lg border shadow-xs">
          {rows.map((r) => (
      ...
      {canManage ? (
        <form
          ref={formRef}
          action={action}
          className="bg-card grid gap-3 rounded-lg border p-4 shadow-xs sm:grid-cols-2"
        >
          <input type="hidden" name="employeeId" value={employeeId} />
          <Field label="Status">
...
function Empty({ title, body }: { title: string; body: string }) {
  return (
    <div className="bg-card flex flex-col items-center gap-2 rounded-lg border border-dashed px-6 py-14 text-center">
