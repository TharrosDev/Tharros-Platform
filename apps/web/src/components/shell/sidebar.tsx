...
function NavLink({ item, ns, onNavigate }: { item: NavItem; ns: string; onNavigate?: () => void }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "rounded-lg min-h-control-lg relative flex items-center gap-2.5 px-2 transition-colors lg:min-h-11",
        active
    ...
    >
      {active ? (
        <m.span
          ...
          className="rounded-lg bg-sidebar-accent absolute inset-0"
          aria-hidden
        />
      ) : null}
