...
function PortalShell({ className, children }: React.ComponentProps<"main">) {
  return (
    <main
      className={cn(
        "bg-background mx-auto flex min-h-dvh w-full max-w-md flex-col px-4 py-4 sm:px-6 sm:py-7 lg:px-8 lg:py-9",
        className,
      )}
    >
      {children}
  ...
  return (
    <header className="mb-8 flex min-h-12 items-center justify-between gap-4 border-b pb-3">
      <Link
        ...
        className="focus-visible:ring-ring/40 rounded-md outline-none focus-visible:ring-[3px]"
      >
        <TharrosWordmark />
      </Link>
      {children}
