  ...
  if (message.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="bg-card text-foreground max-w-[85%] rounded-br-md border px-4 py-2.5 text-[0.9375rem] leading-relaxed shadow-xs whitespace-pre-wrap">
          {message.content}
        </div>
      </div>
    );
...
function AssistantTurn({ message, streaming }: { message: ChatMessage; streaming: boolean }) {
  ...
  return (
    <div className="flex gap-3">
      <span
        aria-hidden
        className="text-primary-soft-foreground bg-card mt-0.5 flex size-7 shrink-0 items-center justify-center border shadow-xs"
      >
