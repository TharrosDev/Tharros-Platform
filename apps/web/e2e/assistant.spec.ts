...
test("assistant spine", async ({ page }) => {
  ...
  await test.step("with no documents, the assistant nudges you to add some", async () => {
    ...
    await expect(
      page.getByText("Add your documents to get started"),
    ).toBeVisible();
  });
  ...
  await test.step("a persisted cited answer renders with its source footer", async () => {
    ...
    const conversationId = await db.seedConversationWithCitedAnswer(
      ownerOrgId,
      ownerId,
      {
    ...
    await expect(
      page.getByText("How long do customers have to return an item?"),
    ).toBeVisible();
    await expect(
  ...
  await test.step("a real question returns a grounded, cited answer", async () => {
    await page.goto("/assistant");
    await page.getByLabel("Ask the assistant a question").fill(
