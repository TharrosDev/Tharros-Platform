...
on every Checkout Session) rejects the session with _"You must have a valid head
office address to enable automatic tax calculation"_ until the **head-office
...
A registration alone is _not_ enough — Stripe won't _collect_ tax until a
registration is active, but it won't even _create the session_ without the
head-office address.
...
only _after_ successful handling, so a failure returns 500 and Stripe retries
